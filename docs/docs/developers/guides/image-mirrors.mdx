...
pagination_label: Image Loading & Mirrors
sidebar_label: Image Loading & Mirrors
description: Reliable image loading with content-node mirror fallback
...
- **mirrors**: Alternate content-node host roots. Mirror order is arbitrary; try each until one
  succeeds.
