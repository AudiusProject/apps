...
To upload a cover image, use the [Uploads API](/developers/sdk/uploads). Upload your image first to
obtain a CID, then pass it as `coverArt` in the `metadata` object.
...
Returns a `Promise` containing an object with a `data` field.
...
[Uploads API](/developers/sdk/uploads) and pass the new CID in `metadata`.
