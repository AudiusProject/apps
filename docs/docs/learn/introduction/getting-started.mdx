...
a free, open, Spotify-style API where the music is permissionless and the ecosystem is yours to
build on.

For protocol-level development (validators, nodes, staking mechanics, DDEX, and the underlying
network), visit [docs.openaudio.org](https://docs.openaudio.org).
