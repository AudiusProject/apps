...
export const updatePlaylistArtwork = async (
  ...
  if (tracksForImage) {
    if (tracksForImage.length === 0) {
      ...
      // Set a temporary cover_art_sizes value so the publish button is enabled immediately
      // The actual CID will be set when the artwork is uploaded
      // Preserve existing cover_art_sizes if it exists, otherwise set a placeholder
      if (!collection.cover_art_sizes && artwork?.file) {
        // Setting temporary placeholder for optimistic update
        // This enables the publish button immediately; actual CID set on upload
        ;(collection as any).cover_art_sizes = 'pending'
      }
    }
  }
