/**
 * Build the Audius OAuth URL for the WebView. Matches the SDK's OAuth.ts flow;
 * the SDK's oauth is browser-only (window), so we build the URL here for React Native.
 * See: packages/sdk/src/sdk/oauth/OAuth.ts and types.ts (OAUTH_URL, params).
 */
