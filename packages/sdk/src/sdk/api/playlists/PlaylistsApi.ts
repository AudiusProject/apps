...
import { UploadTrackSchema } from '../tracks/types'
...
export class PlaylistsApi extends GeneratedPlaylistsApi {
  ...
  override async createPlaylist(
    ...
    if (params.imageFile) {
      const imageUploadResponse = await this.storage
        .uploadFile({
        ...
        .start()
      params.metadata.coverArtCid = imageUploadResponse.orig_file_cid
    }
  ...
  async uploadPlaylist(
    ...
    const { userId, onProgress, audioFiles, imageFile } = await parseParams(
      'uploadPlaylist',
      UploadPlaylistSchema
    )(params)

    const trackMetadatas = params.trackMetadatas
    const playlistMetadata = params.metadata
    ...
    const trackIds = await Promise.all(
      trackMetadatas.map(async (t, i) => {
        ...
        const trackMetadata = this.combineMetadata(
          this.trackUploadHelper.transformTrackUploadMetadata(t, userId),
          playlistMetadata
        )
        ...
        if (this.entityManager) {
          const { metadata } = UploadTrackSchema.parse(updatedMetadata)
  ...
  private combineMetadata(
    trackMetadata: CreateTrackRequestBody,
    playlistMetadata: CreatePlaylistRequestBody
  ) {
