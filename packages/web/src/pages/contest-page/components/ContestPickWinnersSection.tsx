...
const ORIGINAL_SNIPPET_PAGE_SIZE = 1
const MAX_WINNERS = 5

export type ContestPickWinnersSectionHandle = {
  requestFinalize: () => void
}

type ContestPickWinnersSectionProps = {
  trackId: number
  /** When true, shows Finalize at the top of the section (contest page embed). */
  showEmbeddedFinalizeButton?: boolean
  /** Called after a successful save + analytics; use for navigation. */
  onFinalizeNavigate: () => void
  /** Fires when finalize eligibility changes (for standalone page header). */
  onCanFinalizeChange?: (canFinalize: boolean) => void
}

/**
 * Pick-winners experience shared by the standalone Pick Winners page and the
 * desktop contest page submissions tab. Includes the original track, winner
 * ordering UI, and filtered submission pool with +/- adornments.
 */
export const ContestPickWinnersSection = forwardRef<
  ContestPickWinnersSectionHandle,
  ContestPickWinnersSectionProps
>(function ContestPickWinnersSection(
  {
    trackId,
  ...
  const originalSnippet = useRemixesLineup({
    trackId,
    includeOriginal: true,
    includeWinners: false,
    pageSize: ORIGINAL_SNIPPET_PAGE_SIZE,
    sortMethod: 'recent',
    isCosign: false,
  ...
  useEffect(() => {
    onCanFinalizeChange?.(canFinalize)
  }, [canFinalize, onCanFinalizeChange])
  ...
  useImperativeHandle(
    ref,
    () => ({
      requestFinalize: openConfirmationModal
    }),
  ...
  const winnerTileUid = useCallback(
    (tileId: ID) =>
      `kind:${Kind.TRACKS}-id:${tileId}-source:${QueueSource.PICK_WINNERS_TRACKS}:${trackId}`,
    [trackId]
  )
  ...
  const submissionHeading = useCallback((n: number | undefined) => {
    return `${messages.remixesTitle}${n !== undefined ? ` (${n})` : ''}`
  }, [])
  ...
  const tileAdornment = (elementId: ID) => {
  ...
  const renderWinnerTile = useCallback(
    (winnerId: ID, index: number) => {
      ...
      const handleDrop = (droppedId: ID) => {
        const originIdx = winners.indexOf(droppedId)
        const placeIdx = winners.indexOf(winnerId)

        if (droppedId === winnerId || originIdx === placeIdx + 1) return
  ...
  return (
    <Flex column gap='3xl'>
      ...
      <Flex column gap='xl'>
        ...
        <TanQueryLineup
          data={originalSnippet.data}
          isFetching={originalSnippet.isFetching}
          isPending={originalSnippet.isPending}
          isError={originalSnippet.isError}
          hasNextPage={!!originalSnippet.hasNextPage}
          play={originalSnippet.play}
          pause={originalSnippet.pause}
          loadNextPage={originalSnippet.loadNextPage}
          isPlaying={originalSnippet.isPlaying}
          lineup={originalSnippet.lineup}
          pageSize={ORIGINAL_SNIPPET_PAGE_SIZE}
